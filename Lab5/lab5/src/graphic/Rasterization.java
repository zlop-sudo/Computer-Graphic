...
//	}
	...
	// shadow - phong shading
	public void shadow(Illumination illu, Texture t, Shadow s, double groundz) {
		...
		for (int i = 0; i < polygons.length; i++) {
			...
			for (int y = 0; y < size; y++) {
				...
				for (int j = 0; j < aet.size(); j+= 2) {
					...
					double[] left = aet.get(j);
					double[] right = aet.get(j + 1);
					update_buffer(left, right, y, illu, t, s, groundz);
				}
			}
		}
	}
	...
	// update z-buffer and color-buffer using one odd and one even points (version for textureMapping)
	public void update_buffer(double[] left, double[] right, int y, Illumination illu, Texture t, Shadow s, double groundz) {
		double left_x = left[1];
		double left_z = left[3];
		double right_x = right[1];
		double right_z = right[3];
		...
		for (int i = (int) left_x + 1; i <= (int) right_x; i++) {
			...
			pixel_position[2] = left[13] + ((double) i - left_x) * k_z;
			int[] color = new int[3];
			color = t.textureMapping(pixel_position[0], pixel_position[1]);
			if (s.isShadow(pixel_position) && (pixel_position[2] - groundz) < Math.abs(groundz) / 2) {
				color[0] = color[0] / 2;
				color[1] = color[1] / 2;
				color[2] = color[2] / 2;
			}
